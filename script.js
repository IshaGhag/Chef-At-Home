// ===================== CONFIG =====================
const SPOONACULAR_KEY = 'e433c7acbbc9472890884e2feb01c443';
...
function loadCategory(cuisine) {
    ...
    fetch(`${SPOONACULAR_BASE}/recipes/complexSearch?cuisine=${encodeURIComponent(cuisine)}&number=12&addRecipeInformation=true&apiKey=${SPOONACULAR_KEY}`)
        ...
        .catch(() => {
            grid.innerHTML = '<p style="color:var(--text-muted);grid-column:1/-1;text-align:center">Could not load category. Please try again.</p>';
        });
}
loadCategory('Indian');
...
function createCard(meal) {
    ...
    const cuisine = meal.cuisines ? meal.cuisines[0] : (meal.strCategory || '');
